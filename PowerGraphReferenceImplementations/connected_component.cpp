...
struct gather_type : graphlab::IS_POD_TYPE {
  int label;
  gather_type(int label = INT_MAX) : label(label) {}

  gather_type& operator+=(const gather_type& other) {
    label = std::min(label, other.label);
    return *this;
  }
};

/**
 * \brief The connected component vertex program.
 */
class cc :
...
public:
  ...
  //  */
   gather_type gather(icontext_type& context, const vertex_type& vertex,
     ...
     return gather_type(edge.source().data());
  ...
  void apply(icontext_type& context, vertex_type& vertex,
             const gather_type& min_label) {
    int newVertexVal = std::min(vertex.data(), min_label.label);
    if (min_label.label >= vertex.data())
      changed = false;
    else
      changed = true;
  ...
  /**
   * \brief Determine if CC should run on all edges or just in edges
   */
...
}; // end of connected component vertex program
 ...
 */
struct connected_component_writer {
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options
    clopts("Connected Component Algorithm.");
  ...
  // Running The Engine -------------------------------------------------------
  graphlab::omni_engine<cc> engine(dc, graph, exec_type, clopts);

  // Signal all the vertices to start
  ...
  if (saveprefix != "") {
    graph.save(saveprefix, connected_component_writer(),
               false,    // do not gzip
               true,     // save vertices
